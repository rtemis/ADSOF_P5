...
public class GrafoGOT<T extends PersonajeGOT> extends GrafoNoDirigido<T>{
	...
	public List<String> casas() {
		Predicate<String> casa = (s) -> !s.equals("null");
		return vertices.values().stream().map(v -> v.getDatos().getCasa()).filter(casa).distinct().sorted().collect(Collectors.toList());
	}
	...
	public List<String> miembrosCasa(String casa) {
		Predicate<? super Vertice<T>> i = (v) -> v.getDatos().getCasa().equals(casa);
		return vertices.values().stream().filter(i).map(v -> v.getDatos().getNombre()).collect(Collectors.toList());
	}
